package com.knu.ynortman.bits;

public class MultiplicationModuloOperator implements BinaryOperator {
    @Override
    public BitArray combine(BitArray operand1, BitArray operand2) {
        if(operand1.size() != 16 || operand2.size() != 16) {
            throw new IllegalArgumentException("MultiplicationModuloOperator operates only on 16 bits arrays");
        }
        ...
        long res = ((a * b) % (BitArray.MODULO+1));

        BitArray result = null;
        if(res == BitArray.MODULO) {
