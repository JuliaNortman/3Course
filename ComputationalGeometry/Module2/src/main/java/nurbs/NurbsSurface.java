...
public class NurbsSurface {
    ...
    private int n = 1;
    private int m = 3;
    private static final int DEGREE_P = 3;
    private static final int DEGREE_Q = 3;

    private static Point3D[][] controlPoints = new Point3D[][] {
                    {
                            new Point3D(1, 1, 0), new Point3D(2, 2, 0),
                            new Point3D(3, 2, 1), new Point3D(4, 4, 2)
                    },
                    {
                            new Point3D(5, 1, 2), new Point3D(6, 4, 3),
                            new Point3D(7, 2, 4), new Point3D(9, 1, 5)
                    },
            };

    private static double[][] controlPointWeight = {
            {Math.sqrt(2), Math.sqrt(8), Math.sqrt(14), 6},
            {Math.sqrt(30), Math.sqrt(61), Math.sqrt(69), Math.sqrt(107)}
    };
    ...
    private Point3D s(double u, double v){
     ...
     * */
    private double nU(int i, int p, double u){
        if (p == 0){
     ...
     * Defined for u and for v separately because of different knot vectors.
     * */
    private double nV(int i, int p, double v){
        if (p == 0){
