...
public class MainPanel extends JPanel {
    ...
    MainPanel(ArrayList<Target> targets, Hunter hunter){
        ...
        this.hunter = hunter;
        try {
            backgroundImage = new ImageIcon("src/main/resources/swamp.jpg").getImage();
                    //javax.imageio.ImageIO.read(new File("swamp.jpg"));
            //targetImage = new ImageIcon("src/main/resources/UFO-dancing.gif").getImage();
            targetImage = new ImageIcon("src/main/resources/duck.png").getImage();
            bulletImage = new ImageIcon("src/main/resources/bomb.gif").getImage();
                    //javax.imageio.ImageIO.read(new File("bang.png"));
            hunterImage = new ImageIcon("src/main/resources/hunter.png").getImage();
        } catch (Exception e) {
            e.printStackTrace();
        }
...
//                if (new Rectangle(b.getX(), b.getY(), MainPanel.targetWidth, MainPanel.targetHeight).contains(new Rectangle(hunter.getBullets().get(i).getX(), hunter.getBullets().get(i).getY(), 20, 20))) {
//                    b.setAlive(false);
//                    hunter.getBullets().get(i).stopThread();
...
//
//            for (int i = 0; i < 20; i++) {
//                int x = new Random().nextInt((HuntHandler.width - 2 * MainPanel.targetWidth));
//                int y = new Random().nextInt((HuntHandler.height - 2 * MainPanel.targetHeight));
        ...
        if (targetImage != null) {
            for (int i = 0; i < targets.size(); i++) {
                if (targets.get(i).getAlive())
                    g.drawImage(targetImage, targets.get(i).getX(), targets.get(i).getY(), targetWidth, targetHeight, this);
                else {
                    synchronized (targets){
                        targets.remove(i);
                    }
